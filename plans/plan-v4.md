# theabernathypack.com — Implementation Plan (v4)

> **Revision notes (v4):** One change from v3, scoped to M1 (Auth & Login Entity). While implementing M1, research against current Auth.js/NextAuth documentation (not training-data assumptions) turned up a real, documented incompatibility: **NextAuth's Credentials provider only supports the JWT session strategy out of the box** — database sessions + Credentials throws `CALLBACK_CREDENTIALS_JWT_ERROR: "Signin in with credentials only supported if JWT strategy is enabled"`. The only way to force the v3-specified combination (Credentials + database sessions) is an unofficial community workaround (overriding `jwt.encode`/`jwt.decode` to smuggle a DB session token), which relies on undocumented internals that could break on any `next-auth` upgrade — not an acceptable foundation for a portfolio project. **Resolution: Auth.js is dropped. M1 hand-rolls credentials auth with real database-backed sessions** (bcryptjs password hashing, a random session token whose SHA-256 hash is stored in a new `sessions` table, httpOnly cookie holding the raw token). This satisfies the plan's actual intent — database-backed sessions, needed for immediate revocation on account deactivation — without depending on unofficial library internals. Auth.js can be reconsidered later specifically when the brief's deferred Google OAuth SSO item is picked up, since OAuth is the scenario Auth.js is actually designed for. This revision updates §2's Auth row, adds §3.9/§3.10 for the two new tables (`sessions`, `password_reset_tokens`), and updates §6's M1 bullets to describe what was actually built. Nothing else changed from v3 — the AWS architecture, IAM policy, and all other milestones are unaffected and copied forward unchanged. Every section below is complete and current — nothing in this document depends on v1, v2, or v3.

> **DOCUMENT VERSIONING RULE — READ BEFORE EDITING OR EXECUTING THIS PLAN**
>
> This plan will go through multiple review rounds. Every revision MUST be written to a **new file** (`plan-v5.md`, `plan-v6.md`, …) in this same `plans/` directory — never edit a prior version in place, and never delete prior versions.
>
> Each new version **MUST be fully self-contained**: it must restate every piece of information needed to execute the plan from scratch — tech stack, data model, AWS architecture, IAM policy, milestone tasks, everything — as if it were the *only* document in existence. It must **NOT** say things like "see v3 for the schema" or "unchanged from the previous version" or otherwise require the reader to open an older version to understand or execute any part of the plan. Copy forward everything that still applies, in full, and update whatever changed. Treat each version as a total rewrite, not a diff.
>
...
### 3.10 `password_reset_tokens` *(new in v4)*
...
A successful reset deletes all outstanding reset-token rows for that login (not just the one used) and all of that login's `sessions` rows, so a password reset also logs out every existing session.
...
- Hand-rolled credentials auth (no Auth.js — see §2's v4 note): registration, login, logout, password reset (emailed token via Nodemailer/Gmail OAuth2), account deactivation.
...
- No global route-protection middleware/proxy in M1 — `/account` is the only protected route, gated with a page-level `getSession()` + `redirect()` check. (Next.js 16 renamed `middleware.js` to `proxy.js`; revisit with a real `proxy.ts` once M2+ adds more protected routes.)
- Minimal server-rendered header (Login/Register vs. Account links) added to the root layout so the flow is reachable in a browser — not the real responsive nav, which is M6 scope.
...
- SSO/OAuth login (Google, etc.) — note (v4): this is also now the trigger for reconsidering Auth.js/NextAuth, since OAuth is the case it's designed for
