# theabernathypack.com — Implementation Plan (v2)

> **Revision notes (v2):** Fixed Blocker 1 from review — the `iam:PassRole` condition in Section 5.2 was missing `build.apprunner.amazonaws.com` (the principal App Runner's *access role* actually uses to pull images from ECR) and `scheduler.amazonaws.com` (needed for EventBridge Scheduler to assume its execution role). The previous, incorrect `apprunner.amazonaws.com` entry has been replaced rather than merely supplemented, since that principal isn't actually used for role assumption anywhere in this architecture. No other sections changed from v1.
